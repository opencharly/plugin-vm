...
// the deleted host-builder injected (saveDeployConfigNodeForm + the nil reader):
//   - vmLoadDeployConfig: the cycle-free loader-backed reader (loaderkit.LoadHostDeployConfigViaExecutor,
...
//     saveDeployConfigNodeForm fed deploykit.MarshalDeployNode via loaderThreaded().Primaries).
//
// The THIRD primitive the host-builder used to inject — the process-shared deploy-config flock —
// is no longer injected at all: both writers run inside deploykit.MutateDeployConfig, THE one
...
// vmLoadDeployConfig is the plugin's loader-backed reader for the per-host deploy overlay — the
// cycle-free loaderkit.LoadHostDeployConfigViaExecutor read (placement-invariant, works identically
...
func vmLoadDeployConfig() (*deploykit.DeployConfig, error) {
...
// fetchLoaderPrimaries reads, and the host's deleted saveDeployConfigNodeForm fed
...
// vmMarshalNode builds the per-entry node-form marshal callback deploykit.SaveDeployConfig takes.
// It resugars each plan step via the loader-threaded Primaries snapshot (vmFetchLoaderPrimaries) —
// the SAME marshal the deleted host saveDeployConfigNodeForm performed (deploykit.MarshalDeployNode,
// primaries threaded as DATA).
func vmMarshalNode() func(string, *deploykit.DeployNode) (*yaml.Node, error) {
...
// vmSaveDeployConfig persists dc PLUGIN-SIDE via deploykit.SaveDeployConfig — the save callback
...
// deploykit.MutateDeployConfig's, shared with every other overlay writer).
