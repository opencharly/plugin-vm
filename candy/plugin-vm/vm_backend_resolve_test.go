...
// session daemon socket is available, resolveVmBackendPlugin returns a
...
func TestResolveVmBackendPlugin_ExplicitLibvirtMissingSocket(t *testing.T) {
	// Point XDG_RUNTIME_DIR at a temp dir that has no libvirt sockets.
	// vmshared.LibvirtSessionSocketWithProbes() reads this env var.
	t.Setenv("XDG_RUNTIME_DIR", t.TempDir())
	stubNoLibvirtSpawn(t)

	_, err := resolveVmBackendPlugin("libvirt")
	if err == nil {
...
// TestResolveVmBackendPlugin_AutoFallsThroughToQemu asserts that the existing
...
// is the behavior every existing project ships today; the explicit-
// libvirt gate above is opt-in via deploy.yml.
func TestResolveVmBackendPlugin_AutoFallsThroughToQemu(t *testing.T) {
	t.Setenv("XDG_RUNTIME_DIR", t.TempDir())
	stubNoLibvirtSpawn(t)
	backend, err := resolveVmBackendPlugin("auto")
	if err != nil {
...
// TestResolveVmBackendPlugin_ExplicitQemuShortCircuits asserts the qemu
// branch doesn't even probe libvirt — operators on libvirt-less hosts
// (CI runners, containers) get a clean qemu path.
func TestResolveVmBackendPlugin_ExplicitQemuShortCircuits(t *testing.T) {
	t.Setenv("XDG_RUNTIME_DIR", t.TempDir())
	backend, err := resolveVmBackendPlugin("qemu")
	if err != nil {
